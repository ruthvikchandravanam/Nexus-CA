...
ADMIN_MAKER submits a request to create a new Root CA. ADMIN_CHECKER reviews and approves or rejects. On approval, the Crypto API generates a keypair, self-signs a Root CA certificate, and the public certificate becomes downloadable by all authenticated users.
...
- At least one ACTIVE ADMIN_CHECKER who did not submit the request exists in the system (per [BRD — Checker Availability](../BRD.md#checker-availability)).
...
```mermaid
flowchart TD
    A[ADMIN_MAKER fills creation form] --> B{Form validation<br/>passes?}
    B -- No --> B1[Show inline errors] --> A
    B -- Yes --> C[Submit request]
    C --> D[Request PENDING_APPROVAL]
    D --> E[ADMIN_CHECKER opens request]
...
|---|---|---|---|
| 1 | ADMIN_MAKER | Open *Create Root CA* form | Role check enforced by JWT claim. |
| 2 | ADMIN_MAKER | Fill required creation fields: CN, O, C, Key Algorithm, Key Size, Validity Period (years) | CN non-empty, ≤ 64 chars; O non-empty, ≤ 64 chars; C is a valid ISO 3166-1 alpha-2 code; Key Algorithm ∈ Allowed Key Algorithms; Key Size ≥ configured minimum for the chosen algorithm; Validity Period > 0 and ≤ configured CA maximum. |
| 3 | ADMIN_MAKER | Submit | Server re-validates all fields. On any failure, returns 400 with field-level errors. On success, persists a new Request row with status `PENDING_APPROVAL`. |
...
| 5 | ADMIN_CHECKER | Open the request from queue | Self-approval check: if checker is the same user as the maker, the Approve and Reject controls are disabled. |
| 6 | ADMIN_CHECKER | Review payload (no Before snapshot — create operation) | UI renders empty Before pane per [checker-review.md](../checker-review.md). |
| 7a | ADMIN_CHECKER | Click Approve, optional comment | Status transitions `PENDING_APPROVAL → APPROVED`. Notify maker. |
| 7b | ADMIN_CHECKER | Click Reject, mandatory comment | Status transitions `PENDING_APPROVAL → REJECTED`. Notify maker. Flow ends. |
...
| No ACTIVE ADMIN_CHECKER exists | Request remains `PENDING_APPROVAL` (per [BRD — Checker Availability](../BRD.md#checker-availability)). Submission still succeeds; UI shows a warning banner. |
...
|---|---|---|
| Request submitted | All ACTIVE ADMIN_CHECKER users | *Action Required* |
| Approved | Maker | *Request Approved* |
| Rejected | Maker | *Request Rejected* |
| Executed | Maker | *Request Executed* |
| Pending > escalation threshold | All ACTIVE ADMIN_CHECKER users | *Pending Escalation* |
